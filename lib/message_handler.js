...
import maybeTranslateToAr from './any_to_eng';
...
export default function initialize(traductor, botId) {
  ...
  traductor.on('message', function({ type, channel, text, username, user, subtype, ts }) {
    ...
    if (type === 'message' && username !== 'c3po' && subtype !== 'message_changed' && !suspendedChans.has(channel)) {
      ...
      return maybeTranslateToEng(text1)
